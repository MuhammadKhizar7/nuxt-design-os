# Agent Directives for Design OS (Nuxt)
...
- `/sections/[id]/screen-designs/[name]` — Create screen designs
- Capture screenshots
...
- **Light & Dark Mode**: Use `dark:` variants for all colors. Test that all UI elements are visible and readable in both modes. Nuxt UI provides built-in dark mode support via `useColorMode()`.
...
- **Vue 3 Composition API**: Use `<script setup>` syntax with TypeScript for all components.
...
- **Nuxt UI v4**: We use Nuxt UI v4 which provides pre-built components and design system integration.

- **Use Nuxt UI Components**: Prefer Nuxt UI components (`<UButton>`, `<UCard>`, `<UInput>`, etc.) over custom implementations.

- **No Class Merging Utility Needed**: Nuxt UI handles class merging internally. No need for `cn()` or similar utilities.

- **Semantic Colors**: Use Nuxt UI's semantic color system (`primary`, `secondary`, `neutral`, etc.) configured in `app.config.ts`.
...
- **Tailwind CSS v4**: We use Tailwind CSS v4 (included with Nuxt UI). Do not reference or create v3 patterns.

- **No tailwind.config.js**: Tailwind CSS v4 does not use a `tailwind.config.js` file. Configuration is done via CSS `@theme` directive.
...
The `/export` route generates a complete handoff package:
...
- **Portable components**: Props-based Vue components, ready for any Nuxt/Vue setup
