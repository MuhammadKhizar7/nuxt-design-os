...
- `app/components/shell/AppShell.vue` — Application shell
...
8. List screen design components in `app/components/sections/` and `app/components/shell/`
...
- Finished UI designs (Vue 3 components with full styling)
...
- **DO** wire up the emitted events to your routing and API calls
- **DO** replace sample data with real data from your backend
- **DO** implement proper error handling and loading states
- **DO** implement empty states when no records exist (first-time users, after deletions)
- **DO** use test-driven development — write tests first using `tests.md` instructions
- The components are props/event-based and ready to integrate — focus on the backend and data layer
...
- `UserMenu.vue` — User menu with avatar
...
- `product-plan/shell/components/` — Shell Vue components
...
[List emitted events from defineEmits with descriptions]
...
- `product-plan/sections/[section-id]/components/` — Vue components
...
Copy from `app/components/shell/` to `product-plan/shell/components/`:
...
For each section, copy from `app/components/sections/[section-id]/` to `product-plan/sections/[section-id]/components/`:
...
These test-writing instructions are **framework-agnostic**. Adapt them to your testing setup (Jest, Vitest, Playwright, Cypress, Vue Test Utils, RSpec, Minitest, PHPUnit, etc.).
...
- **@product-plan/sections/SECTION_ID/components/** — Vue components to integrate
...
- Components are portable — they work with any Vue setup
